...
traversalPath = []
startingGraph = {}
startingGraph[player.currentRoom.id] = dict((dir, '?') for dir in player.currentRoom.getExits())

def traverse():
    while True:
        while 'n' in startingGraph[player.currentRoom.id] and startingGraph[player.currentRoom.id]['n'] == '?':
            old_room = player.currentRoom.id
            player.travel('n')
            traversalPath.append('n')
            startingGraph[old_room]['n'] = player.currentRoom.id
            if player.currentRoom.id not in startingGraph:
                startingGraph[player.currentRoom.id] = dict((dir, '?') for dir in player.currentRoom.getExits())
            startingGraph[player.currentRoom.id]['s'] = old_room

        while 's' in startingGraph[player.currentRoom.id] and startingGraph[player.currentRoom.id]['s'] == '?':
            old_room = player.currentRoom.id
            player.travel('s')
            traversalPath.append('s')
            startingGraph[old_room]['s'] = player.currentRoom.id
            if player.currentRoom.id not in startingGraph:
                startingGraph[player.currentRoom.id] = dict((dir, '?') for dir in player.currentRoom.getExits())
            startingGraph[player.currentRoom.id]['n'] = old_room

        while 'e' in startingGraph[player.currentRoom.id] and startingGraph[player.currentRoom.id]['e'] == '?':
            old_room = player.currentRoom.id
            player.travel('e')
            traversalPath.append('e')
            startingGraph[old_room]['e'] = player.currentRoom.id
            if player.currentRoom.id not in startingGraph:
                startingGraph[player.currentRoom.id] = dict((dir, '?') for dir in player.currentRoom.getExits())
            startingGraph[player.currentRoom.id]['w'] = old_room

        
        while 'w' in startingGraph[player.currentRoom.id]:
            old_room = player.currentRoom.id
            player.travel('w')
            traversalPath.append('w')
            startingGraph[old_room]['w'] = player.currentRoom.id
            if player.currentRoom.id not in startingGraph:
                startingGraph[player.currentRoom.id] = dict((dir, '?') for dir in player.currentRoom.getExits())
            startingGraph[player.currentRoom.id]['e'] = old_room
        
        if '?' not in startingGraph[player.currentRoom.id].values():
            if bfs(player.currentRoom.id) != None:
                for room in bfs(player.currentRoom.id):
                    for key, value in startingGraph[player.currentRoom.id].items():
                        if value == room:
                            direc = key
                            player.travel(direc)
...
# Create an empty set to store visited nodes
        ...
        while len(q) > 0:
            ...
            if v not in visited:
                ...
                for neighbor in startingGraph[v].values():
                    # Copy the path
                    path_copy = list(p)
...
print(bfs(player.currentRoom.id))
print(startingGraph)
